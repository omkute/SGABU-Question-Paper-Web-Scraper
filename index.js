...
async function scrapeDirectory(remoteDirPath) {
  ...
  const loggedFiles = [];
  const subdirectories = []; // Collect subdirs to process after files
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    // Handle subdirectories - collect them first
  ...
  // NOW process all subdirectories (tree traversal)
  console.log(`🌳 Processing ${subdirectories.length} subdirectories in: ${remoteDirPath}`);
  for (const subdir of subdirectories) {
    await scrapeDirectory(subdir);
  }
  console.log(`✅ Completed: ${remoteDirPath}`);
}
